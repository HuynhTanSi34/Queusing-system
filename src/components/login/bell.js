import "../style.css";
import belled from "../../components/image/belled.png";
function Bell() {
  return (
    <section className="grey">
      <div className="belled">
        <img src={belled} alt="..." />
      </div>
      <sdiv className="screen6">
        <div className="title6">Thông báo</div>
        <div className="bellItem">
          <div className="man">Người dùng: Nguyễn Thị Thùy Dung</div>
          <div className="gian">Thời gian nhận số: 12h20 ngày 30/11/2021</div>
        </div>
        <div className="bellItem">
          <div className="man">Người dùng: Nguyễn Thiên Chinh</div>
          <div>Thời gian nhận số: 12h20 ngày 30/11/2021</div>
        </div>
        <div className="bellItem">
          <div className="man">Người dùng: Võ Thị Kim Liên</div>
          <div className="gian">Thời gian nhận số: 12h20 ngày 30/11/2021</div>
        </div>
        <div className="bellItem">
          <div className="man">Người dùng: Hoàng Nguyễn Quốc Huy</div>
          <div className="gian">Thời gian nhận số: 12h20 ngày 30/11/2021</div>
        </div>
        <div className="bellItem">
          <div className="man">Người dùng: Võ Ngọc Lan Anh</div>
          <div className="gian">Thời gian nhận số: 12h20 ngày 30/11/2021</div>
        </div>
        <div className="bellItem">
          <div className="man">Người dùng: Nguyễn Thị Trúc Anh</div>
          <div className="gian">Thời gian nhận số: 12h20 ngày 30/11/2021</div>
        </div>
      </sdiv>
    </section>
  );
}
export default Bell;
